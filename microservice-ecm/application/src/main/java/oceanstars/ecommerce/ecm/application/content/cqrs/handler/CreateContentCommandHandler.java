...
public class CreateContentCommandHandler implements ICommandHandler<EcmCreateContentResult, EcmCreateContentCommand> {
   ...
   */
  public CreateContentCommandHandler(EventBus eventGateway, ContentRepository contentRepository) {
  ...
  public EcmCreateContentResult handle(EcmCreateContentCommand command) {
    ...
    // 解包内容原始数据
    final ValueObject raw = new ContentRawDataStrategyContext(command.getType()).unpack(command.getRawData());

    // 构建内容元数据实体
    final Content content = Content.newBuilder(contentType, command.getName())
        // 内容展示名称
