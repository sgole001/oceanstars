...
public class ShadedNettyGrpcServerFactory extends AbstractGrpcServerFactory<NettyServerBuilder> {
   ...
   *
   * @param security          安全配置
   * @param sslContextBuilder Shaded Netty SSL上下文构建器
   */
  ...
  /**
   * 转换gRPC客户端认证枚举类型至Shaded Netty客户端认证枚举类型
   *
   * @param clientAuth gRPC客户端认证枚举类型
   * @return Shaded Netty客户端认证枚举类型
   */
  protected static io.grpc.netty.shaded.io.netty.handler.ssl.ClientAuth of(final ClientAuth clientAuth) {
    return switch (clientAuth) {
      case NONE -> io.grpc.netty.shaded.io.netty.handler.ssl.ClientAuth.NONE;
      case OPTIONAL -> io.grpc.netty.shaded.io.netty.handler.ssl.ClientAuth.OPTIONAL;
      case REQUIRE -> io.grpc.netty.shaded.io.netty.handler.ssl.ClientAuth.REQUIRE;
    };
  }
}
