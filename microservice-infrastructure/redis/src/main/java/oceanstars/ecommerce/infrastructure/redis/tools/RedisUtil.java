...
public final class RedisUtil {
   ...
   * 多数据源RedisTemplate
   */
  private final static Map<String, RedisTemplate<String, Object>> redisTemplates =
  ...
  /**
   * 获取默认数据源RedisTemplate
   *
   * @return Redis操作对象
   */
  public static RedisOperation redisTemplate() {
   ...
   * @return 操作结果
   */
  public static <K, V> List<Object> executeInTransaction(Consumer<RedisOperations<K, V>> consumer) {
  ...
  /**
   * 获取指定数据源RedisTemplate
   *
   * @param source 数据源
   * @return Redis操作对象
   */
  public static RedisOperation redisTemplate(String source) {
    return new RedisOperation(redisTemplates.get(source));
  }
   ...
   * @author Clover
   * @version 1.0.0
   * @since 2024/7/4 18:13
   */
  public static final class RedisOperation {
     ...
     */
    private RedisOperation(RedisTemplate<String, Object> redisTemplate) {
     ...
     * @return true：成功；false：失败
     */
    public boolean expire(String key, LocalDateTime time) {
      return expire(key, Date.from(time.atZone(ZoneId.systemDefault()).toInstant()));
    }
    ...
    public boolean expire(String key, Date time) {
      try {
        if (StringUtils.isNotEmpty(key) && null != time) {
